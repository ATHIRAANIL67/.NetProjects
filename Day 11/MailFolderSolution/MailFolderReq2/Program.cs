...
Now we are going to start creating a folder and add mail to it. Start with creating a folder and use menu-driven approach to add, remove, display details of the mail in the folder.
...
Add a default constructor and a parameterized constructor to take in all attributes in the given order: public Mail(long _id, string _from, string _to, string _subject, string _content, DateTime _receivedDate, double _size)
...
Add a default constructor and a parameterized constructor to take in all attributes in the given order:MailFolder(string _name, List < Mail > _mailList).In constructor pass the _mailList value as an empty list. Only one folder will be present at a time.
            ...
            do
            {
                ...
                switch (ch)
                {
                    case 1:
                        ...
                        Mail mail = new Mail(long.Parse(s[0]), s[1], s[2], s[3], s[4], dt, double.Parse(s[6]));
